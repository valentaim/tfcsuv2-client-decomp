...
public class WorldGenFlowers
{
  public static void generate(World world, Random random, int chunkX, int chunkZ, int flowersPerChunk) {
    int flowerType = (new Random(world.func_72905_C() + (((chunkX >> 7) - (chunkZ >> 7)) * (chunkZ >> 7)))).nextInt(14);
    ...
    for (int i = 0; i < flowersPerChunk; i++) {
      ...
      if (world.func_147437_c(xx, yy, zz) && plantBlock.func_149718_j(world, xx, yy, zz))
      {
        if (plantBlock.canGrowConditions(world, xx, yy, zz, flowerType))
          world.func_147465_d(xx, yy, zz, (Block)plantBlock, flowerType, 2);
      }
    }
  }
}
