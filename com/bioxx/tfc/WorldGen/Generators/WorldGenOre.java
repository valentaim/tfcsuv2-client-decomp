  ...
  implements IWorldGenerator
{
  ...
  public static Map<String, OreSpawnData> oreList = (Map<String, OreSpawnData>)new CaseInsensitiveHashMap();
  ...
  public void generate(Random rand, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
    ...
    for (Map.Entry<String, OreSpawnData> entry : oreList.entrySet()) {
      ...
      ore(osd.type, osd.block, osd.meta, osd.base, osd.rarity, osd.min, osd.max, osd.rnd, osd.SphereXSize, osd.SphereYSize, osd.SphereZSize, osd.VeinWidth, osd.VeinBaseHeight, osd.VeinDownFactor, osd.AreaNumber, osd.AreaMaxDistance, osd.CellSize, name);
    }
  }
  ...
  private void ore(EnumOreGen type, Block block, int meta, Map<Block, List<Integer>> baseRocks, int rarity, int min, int max, int rnd, int SphereXSize, int SphereYSize, int SphereZSize, int VeinWidth, int VeinBaseHeight, int VeinDownFactor, int AreaNumber, int AreaMaxDistance, int CellSize, String name) {
    createOre(type, block, meta, baseRocks, rarity, rnd, this.worldObj, this.random, this.chunkX, this.chunkZ, min, max, SphereXSize, SphereYSize, SphereZSize, VeinWidth, VeinBaseHeight, VeinDownFactor, AreaNumber, AreaMaxDistance, CellSize, name);
  }
  ...
  private static void createOre(EnumOreGen type, Block block, int j, Map<Block, List<Integer>> layers, int rarity, int rnd, World world, Random rand, int chunkX, int chunkZ, int min, int max, int SphereXSize, int SphereYSize, int SphereZSize, int VeinWidth, int VeinBaseHeight, int VeinDownFactor, int AreaNumber, int AreaMaxDistance, int CellSize, String name) {
    if (world.func_72959_q() instanceof com.bioxx.tfc.WorldGen.TFCWorldChunkManager)
    {
      for (Block b : layers.keySet()) {

        for (Iterator<Integer> iterator = ((List)layers.get(b)).iterator(); iterator.hasNext(); ) { int metadata = ((Integer)iterator.next()).intValue();
          ...
          if ((rockLayer1.block == b && (rockLayer1.data2 == metadata || metadata == -1)) || (rockLayer2.block == b && (rockLayer2.data2 == metadata || metadata == -1)) || (rockLayer3.block == b && (rockLayer3.data2 == metadata || metadata == -1)))
          {



            (new WorldGenMinableTFCNew(type, block, j, b, metadata, rarity, rnd, SphereXSize, SphereYSize, SphereZSize, VeinWidth, VeinBaseHeight, VeinDownFactor, AreaNumber, AreaMaxDistance, CellSize, name))
