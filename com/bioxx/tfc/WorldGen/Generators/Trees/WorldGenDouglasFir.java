  ...
  extends WorldGenerator
{
  ...
  public boolean func_76484_a(World world, Random rand, int par3, int par4, int par5) {
      ...
      for (int l = par3 - byte0; l <= par3 + byte0 && flag; l++) {
        ...
        for (int j1 = par5 - byte0; j1 <= par5 + byte0 && flag; j1++) {
          ...
          if (j >= 0 && j < 256) {
            ...
            Block j2 = world.func_147439_a(l, j, j1);
            if (!j2.isAir((IBlockAccess)world, l, j, j1) && !j2.isReplaceable((IBlockAccess)world, l, j, j1))
            {
              flag = false;
            }
          }
    ...
    if (!TFC_Core.isSoil(world.func_147439_a(par3, par4 - 1, par5)) || par4 >= 256 - i - 1) {
      return false;
    }
      ...
      for (int l3 = par3 - x; l3 <= par3 + x; l3++) {
        ...
        for (int l4 = par5 - x; l4 <= par5 + x; l4++) {
          ...
          if ((Math.abs(j4) != 0 || (Math.abs(i5) != 0 && k2 != 0)) && (
            Math.abs(j4) + Math.abs(i5) != x * 2 || (k1 - par4 > i / 2 && k1 - par4 < 4 * i / 5) || k1 - par4 - i / 3 + 2 == 2) && rand


            .nextInt(12) != 0 && world.func_147437_c(l3, k1, l4))
          {
            func_150516_a(world, l3, k1, l4, TFCBlocks.leaves, this.metaID);
          }
        }
      }
    }
    func_150516_a(world, par3, par4 + i, par5, TFCBlocks.leaves, this.metaID);
    for (int l1 = 0; l1 < i; l1++)
    {
      func_150516_a(world, par3, par4 + l1, par5, TFCBlocks.logNatural, this.metaID);
    }
    return true;
  }
}
